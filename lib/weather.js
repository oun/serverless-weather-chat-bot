...
export default class Weather {
  async fetchForecastByLocationAndTime(location, datetime) {
    ...
    if (forecastsInDatetime.length > 0) {
      ...
      return this.mapWeather(forecastsInDatetime[0], city.name);
    }
  }

  async fetchCurrentWeatherByLocation(location) {
    console.log(`Fetching current weather by location ${location}`);
    const url = `http://api.openweathermap.org/data/2.5/weather?q=${location}&units=metric&APPID=${apiKey}`;
    const response = await axios.get(url);
    return this.mapWeather(response.data, response.data.name);
  }

  mapWeather(forecast, location) {
